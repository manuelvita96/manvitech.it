...
class PixTestimonial {
	...
	function render($attr, $content = null) {
		...
		wp_enqueue_style('pixfort-circles-style', PIX_CORE_PLUGIN_URI . 'functions/css/elements/css/circles.min.css');
		...
		if (is_string($image) && substr($image, 0, 4) === "http") {
			...
			if (!empty($image['id'])) {
				$image_html3 = wp_get_attachment_image($image['id'], 'pix-square-sm', false, $img_attrs3);
			} else {
				$image_html3 = wp_get_attachment_image($image, 'pix-square-sm', false, $img_attrs3);
			}
			// if (!empty($image_html3[0])) {
		...
		if ($img_style == 'standard' || $img_style == 'circle_top') {
			...
			if ($image_html3) {
				$testimonial .= '<span class="align-self-center pix-circle pix-bg-custom bg-' . $circle_color . ' circle-48 animate-in shadow-sm" ' . $circle_style . ' data-anim-type="fade-in-left" data-anim-delay="200">' . $image_html3 . '</span>';
			}
