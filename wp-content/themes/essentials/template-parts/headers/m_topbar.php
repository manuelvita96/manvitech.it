...
if(!empty($background&&$background=='custom')){
    if(!empty($custom_background)){
        $cssStyle .= 'background: '.$custom_background.' !important;';
    }
}
if(count($m_topbar_val->m_topbar_1->val)>0){
 ?>
 <div class="pix-topbar pix-header-mobile pix-topbar-normal bg-<?php echo esc_attr( $background ); ?> text-white p-sticky py-22 " style="<?php echo esc_attr($cssStyle); ?>" >
     <div class="container">
         <div class="row">
             <?php
