		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
							...
							if($box_btn_link){
								...
								if(!empty($box_btn_text) && $box_btn_text!='') {
									...
									if(pixCheckIconsAvailable()){
										...
										$output .=  '<i class="ml-2 pixicon-angle-right d-flex align-self-center font-weight-bold pix-hover-right" style="line-height:16px;"></i>';
									}
			...
			if(!empty($menu_item_icon)){
				...
				if(pixCheckIconsAvailable()){
					if(pixCheckIconsEnabled()){
						...
						$menu_item_icon_out = \PixfortCore::instance()->icons->getFontIcon($menu_item_icon, $menu_item_icon_classes);
					}
				} else {
					$menu_item_icon_out = '<i class="'.$menu_item_icon.' '.$menu_item_icon_classes.'"></i>';
				}
			}
			...
			if($depth==0){
				...
				if($menu_item_style!='pix-item-heading'){
					$title = '<span class="pix-dropdown-title '.$add_classes.' text-body-default '.$this->dropTextClasses.'">'.$menu_item_icon_out.do_shortcode($title).$dropdownToggleiconOutput.'</span>';
			...
			if($menu_item_style=='pix-item-heading'){
				...
				if(!$atts['href'] || $atts['href']=='#' || $atts['href'] == ''){
					$menuItem .= '<div class="h6 heading-font mega-col-title text-sm '.$add_classes.' text-heading-default '.$this->dropTextClasses.'" >'.$menu_item_icon_out.$title.'</div>';
				}else{
