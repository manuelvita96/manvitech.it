        ...
        if(function_exists('pixfort_core_plugin')){
            ...
            <div class="pix_post_social <?php echo esc_attr( $classes ); ?>">
                ...
                if(pixCheckIconsEnabled()){
                    ...
                    ?>
                    <i class="pixicon-facebook3 <?php echo is_rtl() ? 'pix-pl-10' : 'pix-pr-10'; ?> text-24"></i>
                    <?php
                }
                esc_attr_e('Share on Facebook', 'essentials'); ?>
                </a>
                <a class="btn btn-link pix-social-twitter d-inline-flex align-items-center text-sm font-weight-bold text-body-default pix-px-10" title="Click to share this post on Twitter" href="http://twitter.com/intent/tweet?text=<?php echo get_the_title().' '. get_the_permalink(); ?>" target="_blank">
                    <?php
                        ...
                        if(pixCheckIconsEnabled()){
                            ...
                            ?>
                            <i class="pixicon-x-twitter <?php echo is_rtl() ? 'pix-pl-10' : 'pix-pr-10'; ?> text-24"></i>
                            <?php
                        }
                        esc_attr_e('Share on X', 'essentials'); ?>
                </a>
            </div>
            <?php
        }
...
if ( ! function_exists( 'pixfort_get_post_excerpt_template' ) ) {
    function pixfort_get_post_excerpt_template($atts = []){
        ...
        if($blog_style_box){
            $item_box = ' bg-white ' .$box_class_names;
        }else{
        ...
        switch ($blog_layout) {
            case "full-img":
                $output .= '<div class="d-inline-block position-relative w-100 h-100">';
                    ...
                    $output .= '<div class="card w-100 h-100 bg-black pix-hover-item '.$box_class_names.' position-relative pix-dark overflow-hidden row no-gutters flex-column flex-md-row">';
                        if($img_src){
                            $output .=$img_src;
                        }else{
                                    ...
                                    $output .= '<div class="w-100">';
                                        $output .= '<div class="entry-meta pix-fade-in d-flex align-items-center">';
                                            $output .= '<div class="flex-fill text-left">';
                                                $output .= '<a href="'.get_author_posts_url(get_the_author_meta('ID')).'" class="pix-post-meta-author text-heading-default font-weight-bold" data-toggle="tooltip" data-placement="'.$tooltipPosition.'" title="By '.$author.'">';
                                                  $output .= $author_img;
                                                $output .= '</a>';
                                            $output .= '</div>';
                                        ...
                                        if(has_post_format(array('video', 'audio', 'link', 'quote' ))){
                                            if(has_post_format('video')){
                                                if(get_post_meta( get_the_ID(), 'pix-post-video', true )){
                                                    ...
                                                    $output .= '<div class="pix-post-format-btn text-center d-inline-block2 d-flex justify-content-center">';
                                                       $output .= '<a href="#" data-aspect="embed-responsive-16by9" aria-label="'.esc_attr('Play', 'essentials').'" data-content="'.htmlspecialchars($res).'" class="pix-video-popup pix-post-format-a pix-post-btn-dark  d-inline-block rounded-circle align-self-center scale shadow-lg shadow-hover-lg bg-black pix-p-10 d-flex align-items-center justify-content-center">';
                                            ...
                                            if(has_post_format('audio')){
                                                ...
                                                if(!empty($iframes)){
                                                    if(!empty($iframes[0])){
                                                        ...
                                                        $res = preg_replace("/[\`]/", "", $iframes[0]);
                                                           $output .= '<a href="#" data-aspect="embed-responsive-16by9" aria-label="'.esc_attr('Play', 'essentials').'" data-content="'.htmlspecialchars($res).'" class="pix-audio-popup pix-post-format-a pix-post-btn-dark d-inline-block rounded-circle align-self-center scale shadow-lg shadow-hover-lg bg-black">';
                                                            if(pixCheckIconsAvailable()){
                                            ...
                                            if(has_post_format('link')){
                                                if(get_post_meta( get_the_ID(), 'pix-post-link', true )){
                                                    $output .= '<div class="pix-post-format-btn text-center d-inline-block">';
                                                       $output .= '<a href="'.get_post_meta( get_the_ID(), 'pix-post-link', true ).'" target="_blank" class=" pix-post-format-a pix-post-btn-dark d-inline-block rounded-circle align-self-center scale shadow-lg shadow-hover-lg bg-black">';
                                                        if(pixCheckIconsAvailable()){
                                    ...
                                    $output .= '<div class="text-right d-flex w-100" style="line-height:0;">';
                                        ...
                                        $output .= '<div class="flex-fill text-right">';
                                            $output .= '<a href="'. get_permalink() .'" class="btn btn-sm p-0 btn-link text-body-default svg-body-default font-weight-bold pix-hover-item">';
                                                ...
                                                $output .= '<span class="'.$margin1.' align-middle pix-hover-right flip-icon-rtl">';
                ...
                $output .= '<div class="overflow-hidden2">';
                    if(!$protected && has_post_format(array('video', 'audio', 'link', 'quote' ))){
                        if(has_post_format('video')){
                            if(get_post_meta( get_the_ID(), 'pix-post-video', true )){
                                $res = get_post_meta( get_the_ID(), 'pix-post-video', true );
                                $output .= '<div class="pix-post-format-btn text-center d-inline-block2 d-flex justify-content-center">';
                                   $output .= '<a href="#" data-aspect="embed-responsive-16by9" aria-label="'.esc_attr('Play', 'essentials').'" data-content="'.htmlspecialchars($res).'" class="pix-video-popup pix-post-format-a d-inline-block rounded-circle align-self-center scale shadow-lg shadow-hover-lg bg-white pix-p-10 d-flex align-items-center justify-content-center">';
                                    $output .= pix_load_inline_svg(get_template_directory().'/inc/images/play_arrow.svg');
                                   $output .= '</a>';
                                $output .= '</div>';
                            }else{
                        ...
                        if(has_post_format('audio')){
                            if(get_post_meta( get_the_ID(), 'pix-post-audio', true )){
                                ...
                                $res = get_post_meta( get_the_ID(), 'pix-post-audio', true );
                                   $output .= '<a href="#" data-aspect="embed-responsive-16by9" aria-label="'.esc_attr('Play', 'essentials').'" data-content="'.htmlspecialchars($res).'" class="pix-audio-popup pix-post-format-a d-inline-block rounded-circle align-self-center scale shadow-lg shadow-hover-lg bg-white">';
                                   if(pixCheckIconsAvailable()){
                                        $output .= \PixfortCore::instance()->icons->getIcon('Line/pixfort-icon-sound-max-1', 24, 'm-0 p-0');
                                    } else {
                                        $output .= '<i class="pixicon-volume-3 m-0 p-0"></i>';
                        ...
                        if(has_post_format('link')){
                            if(get_post_meta( get_the_ID(), 'pix-post-link', true )){
                                $output .= '<div class="pix-post-format-btn text-center d-inline-block">';
                	               $output .= '<a href="'.get_post_meta( get_the_ID(), 'pix-post-link', true ).'" target="_blank" class=" pix-post-format-a d-inline-block rounded-circle align-self-center scale shadow-lg shadow-hover-lg bg-white">';
                                    if(pixCheckIconsAvailable()){
                                        $output .= \PixfortCore::instance()->icons->getIcon('Line/pixfort-icon-link-1', 24, 'm-0 p-0');
                                    } else {
                                        $output .= '<i class="pixicon-link-4 m-0 p-0"></i>';
                    ...
                    $output .= '<div class="d-flex align-items-end w-100">';
                        $output .= '<div class="entry-meta pix-fade-in d-flex align-items-center w-100">';
                            $output .= '<div class="flex-fill text-left">';
                                $output .= '<a href="'.get_author_posts_url(get_the_author_meta('ID')).'" class="pix-post-meta-author text-heading-default font-weight-bold" data-toggle="tooltip" data-placement="'.$tooltipPosition.'" title="'.esc_attr__('By', 'essentials').' '.$author.'">';
                                  $output .= $author_img;
                                $output .= '</a>';
                            $output .= '</div>';
                            if(comments_open()){
                    ...
                    if(has_post_format(array('video', 'audio', 'link', 'quote' ))){
                        if(has_post_format('video')){
                            if(get_post_meta( get_the_ID(), 'pix-post-video', true )){
                                ...
                                <div class="pix-post-format-btn text-center d-inline-block2 d-flex justify-content-center">
                                    <a href="#" data-aspect="embed-responsive-16by9" aria-label="<?php esc_attr_e('Play', 'essentials'); ?>" data-content="<?php echo htmlspecialchars($res); ?>" class="pix-video-popup pix-post-format-a d-inline-block rounded-circle align-self-center scale shadow-lg shadow-hover-lg bg-white pix-p-10 d-flex align-items-center justify-content-center">
                        ...
                        if(has_post_format('audio')){
                            if(get_post_meta( get_the_ID(), 'pix-post-audio', true )){
                                ...
                                <?php $res = get_post_meta( get_the_ID(), 'pix-post-audio', true ); ?>
                                    <a href="#" data-aspect="embed-responsive-16by9" aria-label="<?php esc_attr_e('Play', 'essentials'); ?>" data-content="<?php echo htmlspecialchars($res); ?>" class="pix-audio-popup pix-post-format-a d-inline-block rounded-circle align-self-center scale shadow-lg shadow-hover-lg bg-white">
                                    <?php 
                                    if(pixCheckIconsAvailable()){
                        ...
                        if(has_post_format('link')){
                            if(get_post_meta( get_the_ID(), 'pix-post-link', true )){
                                ?>
                                <div class="pix-post-format-btn text-center d-inline-block">
                	                <a href="<?php echo esc_url( get_post_meta( get_the_ID(), 'pix-post-link', true ) ); ?>" aria-label="<?php esc_attr_e('Open link', 'essentials'); ?>" target="_blank" class=" pix-post-format-a d-inline-block rounded-circle align-self-center scale shadow-lg shadow-hover-lg bg-white">
                                    <?php 
                                    if(pixCheckIconsAvailable()){
        ...
        switch ($blog_type) {
            case 'masonry':
                ...
                if(defined('PIX_CORE_PLUGIN_URI')){
                    wp_enqueue_style( 'pixfort-masonry-style', PIX_CORE_PLUGIN_URI.'functions/css/elements/css/masonry.min.css', false, PIXFORT_THEME_VERSION);
